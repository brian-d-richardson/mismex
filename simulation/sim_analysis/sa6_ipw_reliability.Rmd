...
* mismeasured exposure $\pmb{A}^* = (A^*_1, A^*_2)^T = \pmb{A} + \pmb{\epsilon}$, where $\pmb{\epsilon} = (\epsilon_1, \epsilon_2)^T \sim N(\pmb{0}, \pmb{\Sigma}_e)$, $\pmb{\Sigma}_e = \begin{bmatrix} \sigma^2_e & 0 \\ 0 & \sigma^2_e \end{bmatrix}$.
...
The exposure reliability $rel  = \textrm{Var}(A_i) / \textrm{Var}(A_i^*)$ is varied within $[0.5, 1]$, for $i\in\{1,2\}$. Given $rel$, $\sigma^2_e$ is comptued as $(rel^{-1} - 1) * \textrm{Var}(A_i)$
...
make.plot(param. = 2, y.lim = Inf)
make.plot(param. = 3, y.lim = Inf)
