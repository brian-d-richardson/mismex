...
Below are boxplots of the empirical distributions of estimators for $\textrm{E}\{Y(a)\}$ at exposure values $a = 0,1,2,3,4$. The corrected GLM method has several outliers that make it difficult to assess other estimators.
...
Below are plots excluding the "GLM" method.

```{r fig.width=6, fig.height=6}

gfmla.methods <- c("Oracle G-Formula",
                   "Naive G-Formula",
                   "Corrected G-Formula")

plot.by.a(a. = 0, methods. = gfmla.methods)
plot.by.a(a. = 1, methods. = gfmla.methods)
plot.by.a(a. = 2, methods. = gfmla.methods)
plot.by.a(a. = 3, methods. = gfmla.methods)
plot.by.a(a. = 4, methods. = gfmla.methods)

```

### Tables of Simulation Results

Below is a table for the primary estimand of interest ($a=3$). It includes empirical bias, empirical standard error (ESE), average estimated standard error (ASE), and empirical coverage probability of the 95% CI.

```{r}

tbl <- sim.out.long %>% 
  filter(method %in% c("Naive G-Formula",
                       "Corrected G-Formula")) %>% 
  group_by(n, method, a) %>% 
