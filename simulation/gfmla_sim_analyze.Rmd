...
These plots show all simulation results. They are difficult to read due to some extreme corrected GLM estimates.
...
These plots show zoom in to the relevant simulation results.
...
This plot shows only the simulation setting considered in the g-formula sims in the original Blette submission, with $a=3$, $n=800$, $\sigma_e^2=0.25$, and only includes g-formula results.
...
Below is a table of the setting considered in the g-formula sims in the original Blette submission, with $a=3$, $n=800$, $\sigma_e^2=0.25$. It includes empirical bias, empirical standard error (ESE), average estimated standard error (ASE), and mean squared error.
...
* Variance estimation of the corrected g-formula method doesn't perform very well for $n=800$. It improves for larger sample size ($n=8000$). What to do?

* Is the GLM comparator $\widehat{\textrm{E}}\{Y(a)\} = \textrm{logit}^{-1}(\widehat\beta_0+\widehat\beta_aa)$ fair? A slightly less naive approach would be $\widehat{\textrm{E}}\{Y(a)\} = \textrm{logit}^{-1}\{\widehat\beta_0+\widehat\beta_Aa+\widehat\beta_{L_1}\widehat{\textrm{E}}(L_{1})+\widehat\beta_{L_2}\widehat{\textrm{E}}(L_{2})+\widehat\beta_{A:L_1}a\widehat{\textrm{E}}(L_{1})+\widehat\beta_{A:L_2}a\widehat{\textrm{E}}(L_{2})\}$. The original paper says this method is "in essence assuming that adjusting for $L_1$ and $L_2$ in the models will appropriately control for confounding. While this is not a correct interpretation of these model coefficients (since these methods actually target conditional estimands), this interpretation reflects how adjusted outcome models are often misused in practice." This method performs so poorly that it makes the naive g-formula method (which is really the main comparator of interest) look reasonable.
