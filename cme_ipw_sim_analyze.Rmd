...
* Bivariate continuous exposure $\pmb{A} = (A_1, A_2)^T$ with $A|L$ having multivariate normal distribution $N_2\left(\begin{bmatrix} 0.5L \\ -0.5L \end{bmatrix}, \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}\right)$. 
* mismeasured exposure $\pmb{A}^* = (A^*_1, A^*_2)^T = \pmb{A} + \pmb{\epsilon}$, where $\pmb{\epsilon} = (\epsilon_1, \epsilon_2)^T \sim N(\pmb{0}, \pmb{\Sigma}_e)$, $\pmb{\Sigma}_e = \begin{bmatrix} \sigma_e^2 & 0 \\ 0 & 0 \end{bmatrix}$. Note that $\epsilon_2 \equiv 0$, meaning $A^*_2 = A_2$, or $A_2$ is measured without error.
* Binary outcome $Y$ with $\textrm{E}(Y|\pmb{A},L) =  \gamma_0 + \gamma_1 a_1 + \gamma_2 a_2 + \gamma_3 a_3 + \gamma_4L$, where $\widetilde{\pmb{\gamma}} = (\widetilde\gamma_0, \widetilde\gamma_1, \widetilde\gamma_2, \widetilde\gamma_3)^T = (0.4, 0.1, 0.1, 0.2)^T$.

This data generating process leads to the marginal structural model
$\textrm{E}\{Y(\pmb{a})\} = 0.5 * \textrm{logit}^{-1}(\gamma_0 + \gamma_1 a_1 + \gamma_2 a_2 + \gamma_3 a_3)$, where $\pmb{\gamma} = (\gamma_0, \gamma_1, \gamma_2, \gamma_3)^T = (0.5, 0.1, 0.1)^T$.
...
Below is a table of the empirical bias and empirical standard errors of $\widehat{\pmb{\gamma}}$.
...
tbl <- sim.out.long %>% 
  ...
  summarise(bias = mean(ghat - g.true, na.rm = T),
            ...
            mse = mean((ghat - g.true) ^ 2, na.rm = T)) %>% 
...
setNames(tbl, sub(".+_", "", names(tbl))) %>% 
  ...
  add_header_above(c(" " = 4, "Component 1" = 3, "Component 2" = 3,
