...
# CME Corrected Score IPW Simulations: nonlinear A|L model
...
sim.out <- pbapply::pbvapply(
  ...
  FUN = function(ii) {
    ...
    sim.ipw.nonlinearps(n = sim.in$n[ii],
                        B = sim.in$B[ii],
                        vare = sim.in$vare[ii],
...
write.csv(sim.out, row.names = F,
          paste0("simulation/sim_data/ipw_nonlinearps_data/sd", as.integer(args), ".csv"))
