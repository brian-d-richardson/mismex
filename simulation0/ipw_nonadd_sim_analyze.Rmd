---
title: "MCCS IPW Non-Additive ME Simulation Results"
...
The goal of this program is to evaluate the Monte Carlo corrected score inverse probability weighting (MCCS IPW) estimator in the presence of confounding and measurement error, where some measurement error is non-additive.
...
* mismeasured exposure $\pmb{A}^* = (A^*_1, A^*_2,  A^*_3)^T$, where $A^*_1 = A + \epsilon_1$ for $\epsilon \sim N(0, 0.05)$, $A^*_2 = A\epsilon_2$ for $\epsilon \sim N(1, 0.34)$, and $A^*_3 = A_3$.
...
We assume that $A_1$ and $A_2$ are both subject to additive measurement error with variance $0.05$, while $A_2$ truly has multiplicative measurement error.
...
sim.out.list <- lapply(
  ...
  FUN = function(clust) {
    cbind(clust,
          read.csv(paste0("sim_data/ipw_nonadd_data/sd",
                          clust, ".csv")))
  })
