---
title: "MCCS IPW Case-Cohort Simulation Results"
...
1) Oracle Regression Model
2) Oracle IPW
3) Naive Regression Model
4) Naive IPW
5) Corrected Regression Model
6) Corrected IPW
...
Below is a table of the empirical bias, empirical standard error, mean estimated standard error, and empirical coverage probability of the 95% CIs corresponding to $\widehat{\pmb{\gamma}}$, only for the corrected IPW estimators.

```{r}

tbl <- sim.out.long %>% 
  filter(method == "Corrected IPW") %>% 
  mutate(Param = paste0("$\\gamma_", param, "$"),
         pi.cc = 100 * pi.cc) %>% 
