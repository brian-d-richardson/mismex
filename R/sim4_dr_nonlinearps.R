#' Simulation 4: DR with nonlinear PS model
#'
...
sim4.dr.nonlinearps <- function(
  ...
  ps.formula <- "~L1 + L2 + I(L2^2)"            # propensity score model formula
  ipw.formula <- "~A"                           # ipw.formula
  formula.inc <- "~A*L2"                        # incorrect outcome model
  ps.formula.inc <- "~L2 + I(L2^2)"             # incorrect propensity score model
